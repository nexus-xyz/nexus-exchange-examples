...
| --- | --- | --- |
| Rust | [`sdk-rust/`](./sdk-rust) | [`risk-guard/`](./sdk-rust/risk-guard) — watches one account against exposure, loss and margin limits, and cancels resting orders when one is breached · [`liquidation-watch/`](./sdk-rust/liquidation-watch) — how far each mark can move before maintenance margin breaks, ranked by fragility, next to the venue's own ADL record · [`latency-probe/`](./sdk-rust/latency-probe) — place→ack, place→WS, cancel→ack and cancel→WS latency percentiles from post-only orders that cannot fill, with WS frames that never arrive reported as missing, and a clock offset that states how it was bounded |
| TypeScript | [`sdk-ts/`](./sdk-ts) | [`risk-guard/`](./sdk-ts/risk-guard) — watches one account against exposure, loss and margin limits, and cancels resting orders when one is breached · [`dead-mans-switch/`](./sdk-ts/dead-mans-switch) — arms cancel-on-disconnect, then SIGKILLs its own trading session so you can watch the venue cancel for you · [`bracket-orders/`](./sdk-ts/bracket-orders) — an entry plus native take-profit and stop-loss exits sized to the fill, kept one-cancels-other over the WebSocket, reconciled over REST, and left protected on Ctrl-C · [`twap-executor/`](./sdk-ts/twap-executor) — works one parent order as timed IOC children under a price guard, and survives being SIGKILLed mid-slice by reconciling derived client ids against the venue before it sends again |
| Python | [`sdk-python/`](./sdk-python) | [`risk-guard/`](./sdk-python/risk-guard) — watches one account against exposure, loss and margin limits, and cancels resting orders when one is breached · [`agent-enrollment/`](./sdk-python/agent-enrollment) — delegates trading to a scoped, expiring agent key and takes it back again, reading the EIP-712 signing domain from the venue rather than guessing it · [`ccxt-port/`](./sdk-python/ccxt-port) — runs CCXT-shaped code against Nexus through the adapter, side by side with the native client, measuring what the unified layer costs |
