...
23:57:34    cancelled 2 resting order(s)
```

This is the TypeScript twin of
[`sdk-rust/risk-guard`](../../sdk-rust/risk-guard) — same app, same limits, same
posture — so the two read as a set and the differences are the ones the language
and SDK actually impose.
...
take the signal, the cancel does not, and every request is bounded by the
client's own `timeoutMs`, so "not interruptible" never means "hangs forever". A
second Ctrl-C exits at once and says what that can cost.
