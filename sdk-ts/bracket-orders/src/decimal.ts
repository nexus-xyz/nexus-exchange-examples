...
// SDK types them as `Decimal = string` for the same reason. A bracket needs a
// little arithmetic on them, and none of it may touch a float: comparing a
// trigger against the mark, checking a price is on the tick grid, and summing
// fills into the quantity the exits must cover. That last one is the one that
// bites. An entry that fills `0.001` three times has filled `0.003` only in
// decimal; in IEEE-754 the sum comes out a hair short, and an exit sized from it
// is off the lot grid or leaves dust unprotected.
//
// Copied from `sdk-ts/twap-executor`. Examples are self-contained by design
// (CONTRIBUTING § 1), so it's copied, not imported.
